...
class AE64:
    ...
    def _init_snippets(self):
        ...
        self._initDecoder, _ = self._ks.asm(
            self._initDecoderAsm, as_bytes=True)
        ...
        self._initDecoderSmall, _ = self._ks.asm(
            self._initDecoderSmallAsm, as_bytes=True)
    ...
    def _gen_encoded_small_lvl2_decoder(self, sc: bytes) -> bytes:
        ...
        for i in range(length):
            ...
            if sc[i] < 0x80:
                raise Exception(
                    "this should not happen, please contact author")
            else:
                tmpInfo.reg = 'rdx'
                for j in range(2):
        ...
        for infoPlus in self._encodeInfoPlus:
            if infoPlus.needChangeRdi:
                if infoPlus.needPushByte:
                    decoderAsm += "push {};push rsp;pop rcx;".format(
                        infoPlus.gadget.mul.byte)
                decoderAsm += "imul di, [rcx], {};\n".format(
                    infoPlus.gadget.mul.word)
    ...
    def _gen_small_encoded_shellcode(self, sc: bytes, offset: int) -> AnyStr:
        scLength = len(sc)
    ...
    def _patch_level2_decoder(self, start: int, len_sc: int) -> bytes:
        def get_mul_pair(num: int, no_add: bool = False) -> Tuple[int, int, int]:
            def z3_isalnum(ch):
                return z3.Or(
    ...
    def encode_small(self, shellcode: bytes, register: str = 'rax', offset: int = 0) -> bytes:
        ...
        encoder2Start = offset + \
            len(prologue) + len(decoder) + nopLength + \
            len(encodedLvl2DecoderTemplate)
        encodedLvl2Decoder, encoder2Offset = self._patch_level2_decoder(
